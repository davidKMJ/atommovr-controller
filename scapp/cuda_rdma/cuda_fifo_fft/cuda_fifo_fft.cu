...
cuda_fifo_fft.cu                               (c) Spectrum GmbH , 10/2018

**************************************************************************

Example for all M4i/M4x/M2p analog acquisition cards. 
Shows calculation of FFT on a GPU in FIFO mode without using RMDA.
...
int main ()
    ...
    // ----- DMA BUFFER SETUP -----
    void* pvDMABuffer_host = pvAllocMemPageAligned (lBufferSize);
    if (pvDMABuffer_host == NULL)
        {
        spcm_vClose (hCard);
        return EXIT_FAILURE;
        }
    ...
    // setup DMA transfer
    spcm_dwDefTransfer_i64 (hCard, SPCM_BUF_DATA, SPCM_DIR_CARDTOPC, lNotifySize, pvDMABuffer_host, 0, lBufferSize);

    // ----- get buffer on GPU that will be used as target for data transfer -----
    int lCUDADeviceIdx = 0;         // index of used CUDA device
    if (!bInitCUDADevice (lCUDADeviceIdx))
        {
        spcm_vClose (hCard);

        vFreeMemPageAligned (pvDMABuffer_host, lBufferSize);

        return EXIT_FAILURE;
        }

    void* pvDMABuffer_gpu = NULL;
    cudaError_t eCudaErr = cudaMalloc (&pvDMABuffer_gpu, lBufferSize);
    ...
    eCudaErr = cudaMalloc (&afScaledAndDemuxed_gpu, lNumCh * lSamplesPerCh * sizeof (cufftReal));
...
#ifdef _LINUX64
    // cufft callback are only available on 64bit Linux systems.
    // see https://docs.nvidia.com/cuda/cufft/index.html#callback-overview for details
    ...
    while (true/*qwTotalMem < qwToTransfer*/)
        ...
        if ((dwError = spcm_dwSetParam_i32 (hCard, SPC_M2CMD, M2CMD_DATA_WAITDMA)) != ERR_OK)
            ...
            if (lAvailUser >= lNotifySize)
                ...
                // ----- copy data from DMA buffer on host to GPU -----
                eCudaErr = cudaMemcpy ((char*)pvDMABuffer_gpu + lPCPos, (char*)pvDMABuffer_host + lPCPos, lNotifySize, cudaMemcpyHostToDevice);
                if (eCudaErr != cudaSuccess)
                    {
                    printf ("ERROR in cudaMemcpy(DMAData): %s\n", cudaGetErrorString (eCudaErr));
                    break;
                    }
...
#ifndef _LINUX64
                // on linux this has been done by a FFT callback, but on Windows this is not available
                lBlocks = (lNumFFTSamples * lNumCh) / CUDA_MAX_THREADS_PER_BLOCK;
                lThreadsPerBlock = CUDA_MAX_THREADS_PER_BLOCK;
                CudaScaleFFTResult <<< lBlocks, lThreadsPerBlock >>> ((cufftComplex*)pvBufferFFT_gpu, (cufftComplex*)pvBufferFFT_gpu, lSamplesPerCh);
#endif
                ...
                if (eCudaErr != cudaSuccess)
                    {
                    printf ("ERROR in cudaMemcpy(FFTData): %s\n", cudaGetErrorString (eCudaErr));
                    break;
                    }
