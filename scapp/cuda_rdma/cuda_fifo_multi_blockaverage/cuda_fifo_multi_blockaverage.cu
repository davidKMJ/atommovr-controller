...
int main ()
    ...
    // ----- DMA BUFFER SETUP -----
    // ----- get buffer that will be used as target for DMA transfer -----
    void* pvDMABuffer_card = pvAllocMemPageAligned (lBufferSize_bytes);
    if (pvDMABuffer_card == NULL)
        {
        spcm_vClose (hCard);
        return EXIT_FAILURE;
        }

    // setup DMA transfer from Spectrum card to PC
    spcm_dwDefTransfer_i64 (hCard, SPCM_BUF_DATA, SPCM_DIR_CARDTOPC, lNotifySize_bytes, pvDMABuffer_card, 0, lBufferSize_bytes);

    int lCUDADeviceIdx = 0;         // index of used CUDA device
    bool bCudaInitOk = bInitCUDADevice (lCUDADeviceIdx);
    if (!bCudaInitOk)
        {
        ...
        vFreeMemPageAligned (pvDMABuffer_card, lBufferSize_bytes);
        return EXIT_FAILURE;
        }

    // create buffers on GPU

    // create a buffer that we will use as a target on the GPU for data that has been DMA'd into the PC
    // we process data in chunks of NotifySize_bytes
    void* pvBuffer_gpu = NULL;
    cudaError_t eCudaErr = cudaMalloc (&pvBuffer_gpu, lNotifySize_bytes);
    ...
    void* pvBufferProcessed_gpu = NULL;
    eCudaErr = cudaMalloc (&pvBufferProcessed_gpu, lAveragedSize_bytes);
    if (eCudaErr != cudaSuccess)
        {
        // cleanup
        printf ("ERROR in cudaMalloc(): %s\n", cudaGetErrorString(eCudaErr));
    ...
    while (qwTotalMem < qwToTransfer)
        ...
        if ((dwError = spcm_dwSetParam_i32 (hCard, SPC_M2CMD, M2CMD_DATA_WAITDMA)) != ERR_OK)
            ...
            if (lAvailUser >= lNotifySize_bytes)
                ...
                cudaMemcpy (pvBuffer_gpu, (char*)pvDMABuffer_card + lPCPos, lNotifySize_bytes, cudaMemcpyHostToDevice);

                // start kernel on the GPU to process the transfered data
                const int lThreadsPerBlock = 1024;
                if (lBytesPerSample == 1)
                    CudaKernelBlockAverage <<< (lNotifySize_bytes / sizeof (int8)) / lThreadsPerBlock, lThreadsPerBlock >>> ((int8*)pvBuffer_gpu, (int32*)((char*)pvBufferProcessed_gpu));
                    //CudaKernelBlockAverageNoiseSuppression <<< (lNotifySize_bytes / sizeof (int8)) / lThreadsPerBlock, lThreadsPerBlock >>> ((int8*)pvBuffer_gpu, (int32*)((char*)pvBufferProcessed_gpu), cThreshold, cBaseline);
                else
                    CudaKernelBlockAverage <<< (lNotifySize_bytes / sizeof (int16)) / lThreadsPerBlock, lThreadsPerBlock >>> ((int16*)pvBuffer_gpu, (int32*)((char*)pvBufferProcessed_gpu));
                    //CudaKernelBlockAverageNoiseSuppression <<< (lNotifySize_bytes / sizeof (int16)) / lThreadsPerBlock, lThreadsPerBlock >>> ((int16*)pvBuffer_gpu, (int32*)((char*)pvBufferProcessed_gpu), nThreshold, nBaseline);
