"""Centroid extraction and grid assignment for tweezer-array camera frames.

Pipeline: blob-detect atom centroids in a raw frame (:class:`BlobDetection`),
estimate the grid's rotation with one of several ``estimate_grid_rotation_*``
methods, rectify, then assign centroids back onto the occupancy grid
(:func:`fit_grid_and_assign`). :class:`Extractor` wraps this as one call for
callers that don't need to pick a rotation method themselves.
"""
