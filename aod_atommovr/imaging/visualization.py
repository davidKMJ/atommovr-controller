"""Schematic (non-camera) visualizations of rearrangement move batches.

Replays a round's move batches geometrically over a snapshot occupancy grid
...
from aod_atommovr.imaging.extraction import BlobDetection
...
def _simulate_move_batches(
    occupancy: np.ndarray, move_batches: List[List[Move]]
) -> Tuple[
    List[np.ndarray],
    List[List[Tuple[Tuple[int, int], Tuple[int, int]]]],
]:
    """Replay *move_batches* geometrically over a snapshot *occupancy* grid —
    the simulation shared by every panel/frame renderer in this module. No
    collision/failure detection: unlike ``visualize_batch_moves_on_image``
    below, this has only an occupancy snapshot to work from, not a live,
    physics-aware ``AtomArray``.

    Returns ``(snapshots, movements_per_step)``: ``snapshots[0]`` is the
    initial occupancy; ``snapshots[k]`` (``k>=1``) is the state after
    ``move_batches[k-1]``. ``movements_per_step`` is 0-indexed per batch
    (length ``len(move_batches)``, one entry per transition — one shorter
    than ``snapshots``).
    """
    M = np.asarray(occupancy, dtype=int)
    R, C = M.shape
    ...
    for batch in move_batches:
        intended: List[Tuple[Tuple[int, int], Tuple[int, int]]] = [
            ((int(mv.from_row), int(mv.from_col)), (int(mv.to_row), int(mv.to_col)))
            for mv in batch
        ]

        next_state = snapshots[-1].copy()
        ...
        movements_per_step.append(intended)

    return snapshots, movements_per_step
...
def visualize_move_batches(
    ...
    Parameters
    - occupancy: binary occupancy grid (the round's pre-move snapshot)
    ...
    """
    snapshots, movements_per_step = _simulate_move_batches(occupancy, move_batches)
